...
from pretreat_fields_2D import smooth_edge_2D

working_dir='/scratch/gpfs/MIKHAILOVA/zl8336/Curved_surface/no_curve/45/2D/Initialize_Field'
...
def generate_fields(
    ...
    def transversal_field(xx:np.ndarray,yy:np.ndarray):
        ...
        field=np.power(1+np.square(x_rot/laser_zR),-1/4)*np.exp(-np.square(y_rot/w_Z))*np.exp(-np.square(phi/(laser_omega0*laser_tau)))*np.cos(phi-0.5*Gouy_Z+laser_phase)
        return field
    def longitudinal_E_field(xx:np.ndarray,yy:np.ndarray):
        ...
        phi=laser_k*(x_rot-center_position+Kappa_Z*np.square(y_rot)/2)
        Gouy_Z=np.arctan(x_rot/laser_zR)
        field=np.power(1+np.square(x_rot/laser_zR),-3/4)*(y_rot*np.cos(polarisation_angle_rad)/laser_zR)*np.exp(-np.square(y_rot/w_Z))*np.exp(-np.square(phi/(laser_omega0*laser_tau)))*np.cos(phi-1.5*Gouy_Z-np.pi/2+laser_phase)
        return field
    def longitudinal_B_field(xx:np.ndarray,yy:np.ndarray):
        ...
        phi=laser_k*(x_rot-center_position+Kappa_Z*np.square(y_rot)/2)
        Gouy_Z=np.arctan(x_rot/laser_zR)
        field=np.power(1+np.square(x_rot/laser_zR),-3/4)*(-y_rot*np.sin(polarisation_angle_rad)/laser_zR)*np.exp(-np.square(y_rot/w_Z))*np.exp(-np.square(phi/(laser_omega0*laser_tau)))*np.cos(phi-1.5*Gouy_Z-np.pi/2+laser_phase)
        return field
...
generate_fields(
    ...
    theta_degree=45,
    laser_phase=np.pi/2,
    center_position=x_min*0.6,
)
Ex=smooth_edge_2D(Ex,x<0,edge_length=cells_per_lambda_x)
Ey=smooth_edge_2D(Ey,x<0,edge_length=cells_per_lambda_x)
Ez=smooth_edge_2D(Ez,x<0,edge_length=cells_per_lambda_x)
Bx=smooth_edge_2D(Bx,x<0,edge_length=cells_per_lambda_x)
By=smooth_edge_2D(By,x<0,edge_length=cells_per_lambda_x)
Bz=smooth_edge_2D(Bz,x<0,edge_length=cells_per_lambda_x)
