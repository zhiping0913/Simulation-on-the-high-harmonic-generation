...
working_dir='/scratch/gpfs/MIKHAILOVA/zl8336/Curved_surface/no_curve/45/2D/ND_a0=0.25'
...
def plot_field(Electric_Field:np.ndarray,grid_x_axis=grid_x_axis,name=''):
    assert Electric_Field.ndim==1
    assert grid_x_axis.ndim==1
    assert Electric_Field.size==grid_x_axis.size
    fig,ax = plt.subplots()
    ax.plot(grid_x_axis/laser_lambda_M,Electric_Field/laser_Ec_M)
    ax.set_xlabel('x/λ0')
    ax.set_ylabel('a=E/Ec')
    ax.set_ylim(-1.1*laser_a0,1.1*laser_a0)
...
def get_polarisation(Electric_Field_Ey:np.ndarray,Electric_Field_Ez:np.ndarray,direction=1,name=''):
    ...
    Electric_Field_Ey_energy=np.einsum('i,i->',Electric_Field_Ey,Electric_Field_Ey)*d_x
    Electric_Field_Ez_energy=np.einsum('i,i->',Electric_Field_Ez,Electric_Field_Ez)*d_x
...
def filter_spectrum(Electric_Field_spectrum:np.ndarray,filter_range=(1.5,highest_harmonic),name=''):
    ...
    Electric_Field_filter_spectrum_square=Electric_Field_spectrum_square*frequency_mask
    ...
    Electric_Field_filter_efficiency=np.sum(Electric_Field_filter_spectrum_square)*d_f/laser_energy_M
    ...
    return Electric_Field_filter,Electric_Field_filter_spectrum,Electric_Field_filter_efficiency
...
def get_envelope(Electric_Field:np.ndarray,direction=1,name=''):
    ...
    plt.plot(grid_x_axis/laser_lambda_M,continuation_field(Electric_Field,n_continuation_x=n_field_x)/laser_Ec_M,c='b',label='field',linewidth=2)
    plt.plot(grid_x_axis/laser_lambda_M,Electric_Field_envelope/laser_Ec_M,linestyle='--',c='g',label='envelope',linewidth=1)
    plt.plot(grid_x_axis/laser_lambda_M,-Electric_Field_envelope/laser_Ec_M,linestyle='--',c='g',linewidth=1)
    ...
    #plt.ylim(-Electric_Field_envelope_max/laser_Ec_M,Electric_Field_envelope_max/laser_Ec_M)
    #plt.ylim(-1,1)
    plt.legend()
    plt.xlabel(xlabel='x_L/λ0')
...
def get_phase(Electric_Field:np.ndarray,harmonic_order=1,name=''):
    ...
    Args:
        Electric_Field (np.ndarray): _description_
    ...
    Electric_Field_spectrum_group_delay_position_0_fn=-(Electric_Field_spectrum_group_delay_plus_fn+Electric_Field_spectrum_group_delay_minus_fn)/2
    ...
    Electric_Field_spectrum_intrinsic_phase_0_fn=((((Electric_Field_spectrum_intrinsic_phase_plus_fn-Electric_Field_spectrum_intrinsic_phase_minus_fn)/2)/np.pi) % 2)*np.pi
    ...
    print('φn0=%fπ' %(Electric_Field_spectrum_intrinsic_phase_0_fn/np.pi))
    ...
    ax1.set_ylabel('I(k)/I0',color='red')
    ax1.tick_params(axis='y', labelcolor='red')
    line2,=ax2.plot(freq_x_order_th/laser_f0_M,vacuum_length_x_lambda_M/2+Electric_Field_spectrum_group_delay_order_th/laser_lambda_M,label='group delay', color='blue')
    line3,=ax2.plot(freq_x_order_th/laser_f0_M,vacuum_length_x_lambda_M/2+Electric_Field_spectrum_group_delay_order_th_fit/laser_lambda_M,linestyle='--',label='group delay fit')
    ax2.set_ylim((vacuum_length_x_lambda_M/2+Electric_Field_spectrum_group_delay_order_th_average/laser_lambda_M-2,vacuum_length_x_lambda_M/2+Electric_Field_spectrum_group_delay_order_th_average/laser_lambda_M+2))
    ...
    plt.xlabel(xlabel='k/k0')
    ax2 = ax1.twinx() 
    line1,=ax1.semilogy(freq_x/laser_f0_M,Electric_Field_spectrum_square/laser_spectrum_peak_M**2,label='Spectrum', color='red')
    ax1.set_ylim(1e-6,1.1)
    ax1.set_ylabel('I(k)/I0',color='red')
    ax1.tick_params(axis='y', labelcolor='red')
    line2,=ax2.plot(freq_x/laser_f0_M,Electric_Field_spectrum_intrinsic_phase/np.pi,label='Phase', color='blue')
    ax2.set_ylim(Electric_Field_spectrum_intrinsic_phase_order_th_0/np.pi-1,Electric_Field_spectrum_intrinsic_phase_order_th_0/np.pi+35)
    ax2.set_ylabel('φ/π', color='blue')
    ax2.tick_params(axis='y', labelcolor='blue')
    plt.legend(handles=[line1,line2])
    plt.title('Phase')
    ...
    ax1.set_ylabel('I(k)/I0',color='red')
    ax1.tick_params(axis='y', labelcolor='red')
    line2,=ax2.plot(freq_x[Electric_Field_spectrum_square/laser_spectrum_peak_M**2>1e-3]/laser_f0_M,Electric_Field_spectrum_group_delay[Electric_Field_spectrum_square/laser_spectrum_peak_M**2>1e-3]/laser_lambda,label='group delay', color='blue')
...
#Electric_Field_Ey=read_sdf(sdf_name=os.path.join(working_dir,'0018.sdf'),block_name_list=['Electric_Field_Ey'])['Electric_Field_Ey'][n_field_x:]
#Electric_Field_Ey=reverse_field(Electric_Field_Ey)
Electric_Field_Ey_spectrum,Electric_Field_Ey_spectrum_square=get_spectrum(Electric_Field_Ey,name='Reflection')
pd.DataFrame(data=Electric_Field_Ey,index=grid_x_axis).to_hdf(path_or_buf='/scratch/gpfs/MIKHAILOVA/zl8336/Curved_surface/no_curve/45/centerline.hdf5',key='ND_a0=0.25 2D Reflection Electric_Field_Ey')
pd.DataFrame(data=Electric_Field_Ey_spectrum_square[n_continuation_x//2:],index=freq_x_axis[n_continuation_x//2:]).to_hdf(path_or_buf='/scratch/gpfs/MIKHAILOVA/zl8336/Curved_surface/no_curve/45/centerline.hdf5',key='ND_a0=0.25 2D Reflection Electric_Field_Ey_spectrum_square')
exit(0)
