...
working_dir='/scratch/gpfs/MIKHAILOVA/zl8336/Curved_surface/test_a_-0.025'
...
laser_energy=theoretical_laser_energy(amp=laser_amp,z_length=laser_tau*C.speed_of_light,w_0=laser_w0_lambda*laser_lambda)
laser_spectrum_on_x_peak=math.sqrt(C.pi**3/2)*laser_amp**2*(laser_tau*C.speed_of_light)**2*(laser_w0_lambda*laser_lambda)/4
laser_spectrum_on_y_peak=math.sqrt(C.pi**3/2)*laser_amp**2*(laser_tau*C.speed_of_light)*(laser_w0_lambda*laser_lambda)**2/2
...
def plot_field(Field:np.ndarray,ax=None,grid_x_axis=grid_x_axis,grid_y_axis=grid_y_axis,threshold=0,normalize=laser_Ec,norm=Normalize(vmin=-laser_a0,vmax=laser_a0,clip=True),cmap='RdBu',name=''):
    assert Field.ndim==2
    ...
    Field_masked = np.ma.masked_where(condition=Field_zoom <= threshold, a=Field_zoom)
    pcm=ax.pcolormesh(zoom(grid_x_axis,zoom=zoom_factor)/laser_lambda,zoom(grid_y_axis,zoom=zoom_factor)/laser_lambda,Field_masked,norm=norm,cmap=cmap)
    ax.set_aspect('equal')
    plt.colorbar(pcm).ax.set_ylabel('a=E/Ec')
    plt.xlabel('x/λ0')
    plt.ylabel('y/λ0')
    plt.title(name)
    if return_ax:
        ...
        print(os.path.join(working_dir,'Electric_Field_%s.png' %(name)))
        plt.close(fig)
        plt.clf()
...
def plot_2D_spectrum(Electric_Field_spectrum_square:np.ndarray,freq_x_axis=freq_x_axis,freq_y_axis=freq_y_axis,name=''):
    ...
    print(os.path.join(working_dir,'Electric_Field_spectrum_square_%s.png' %(name)))
    plt.close(fig)
    plt.clf()
...
def filter_spectrum(Electric_Field_spectrum:np.ndarray,filter_range=(1.5,highest_harmonic),name=''):
    ...
    Electric_Field_filter=get_field_from_spectrum(Electric_Field_filter_spectrum,name=name)
    ...
    #plot_field(Electric_Field=Electric_Field_filter)
...
def get_envelope(Electric_Field:np.ndarray,name=''):
    ...
    print('Electric_Field_envelope_max: %f×Ec'%(Electric_Field_envelope_max/laser_Ec))
    ...
    return {
        ...
        'Electric_Field_envelope_max_(x,y)':(grid_x_axis[Electric_Field_envelope_max_id[0]],grid_y_axis[Electric_Field_envelope_max_id[1]]),
    ...
    plt.plot(grid_x_axis/laser_lambda,Electric_Field[:,Electric_Field_envelope_max_id[1]]/laser_Ec,c='b',label='field',linewidth=2)
    plt.plot(grid_x_axis/laser_lambda,Electric_Field_envelope[:,Electric_Field_envelope_max_id[1]]/laser_Ec,linestyle='--',c='g',label='envelope',linewidth=1)
    plt.plot(grid_x_axis/laser_lambda,-Electric_Field_envelope[:,Electric_Field_envelope_max_id[1]]/laser_Ec,linestyle='--',c='g',linewidth=1)
...
def get_envelope_width(Electric_Field:np.ndarray,name=''):
    #assert Electric_Field.shape==(n_field_x,n_field_y)
    Electric_Field_envelope_dict=get_envelope(Electric_Field=Electric_Field,name=name)
    Electric_Field_envelope=Electric_Field_envelope_dict['Electric_Field_envelope']
    Electric_Field_envelope_max=Electric_Field_envelope_dict['Electric_Field_envelope_max']
    ...
    Electric_Field_envelope_centercross_peak_left=map_coordinates(input=grid_y_axis,coordinates=np.array([[Electric_Field_envelope_centercross_peak[2].item()]]),order=1).item()   #unit: m
    Electric_Field_envelope_centercross_peak_right=map_coordinates(input=grid_y_axis,coordinates=np.array([[Electric_Field_envelope_centercross_peak[3].item()]]),order=1).item()   #unit: m
    Electric_Field_envelope_centercross_peak_waist=Electric_Field_envelope_centercross_peak_width/2
    Electric_Field_envelope_centerline_FWHM=Electric_Field_envelope_centerline_peak_width/C.speed_of_light   #unit: s
    print('waist at the peak: %fλ0' %(Electric_Field_envelope_centercross_peak_waist/laser_lambda))
    print('centerline FWHM: %ffs' %(Electric_Field_envelope_centerline_FWHM/C.femto))
    print('Electric_Field_envelope_centercross_peak,(left,right): (%f,%f)λ0'%(Electric_Field_envelope_centercross_peak_left/laser_lambda,Electric_Field_envelope_centercross_peak_right/laser_lambda))
    return {
        'Electric_Field_envelope_max':Electric_Field_envelope_max,
        'Electric_Field_envelope_max_id':Electric_Field_envelope_max_id,
        'Electric_Field_envelope_max_(x,y)':(grid_x_axis[Electric_Field_envelope_max_id[0]],grid_y_axis[Electric_Field_envelope_max_id[1]]),
        'Electric_Field_envelope_centercross_peak_waist':Electric_Field_envelope_centercross_peak_waist,
        'Electric_Field_envelope_centercross_peak_left':Electric_Field_envelope_centercross_peak_left,
        'Electric_Field_envelope_centercross_peak_right':Electric_Field_envelope_centercross_peak_right,
        'Electric_Field_envelope_centerline_FWHM':Electric_Field_envelope_centerline_FWHM,
    }
...
def get_spectrum(Electric_Field:np.ndarray,name=''):
    ...
    print(np.einsum('ij,ij->',Electric_Field,Electric_Field)*d_x*d_x/laser_energy)
...
def get_field_from_spectrum(Electric_Field_spectrum:np.ndarray,name=''):
    assert Electric_Field_spectrum.shape==(n_continuation,n_continuation)
    Electric_Field=np.real(np.fft.ifft2(np.fft.ifftshift(Electric_Field_spectrum))[grid_center_mask])*n_continuation*d_f*n_continuation*d_f
...
def get_evolution(
    ...
    Electric_Field_Ey_evolution_1_list=[]
    Electric_Field_Ey_evolution_3_list=[]
    Electric_Field_Ey_evolution_5_list=[]
    Electric_Field_Ey_evolution_7_list=[]
    for evolution_time in evolution_time_list:
        ...
        Electric_Field_1, _=filter_spectrum(Electric_Field_Ey_evolution_spectrum,(0.5,1.5))
        Electric_Field_3, _=filter_spectrum(Electric_Field_Ey_evolution_spectrum,(2.5,3.5))
        Electric_Field_5, _=filter_spectrum(Electric_Field_Ey_evolution_spectrum,(4.5,5.5))
        Electric_Field_7, _=filter_spectrum(Electric_Field_Ey_evolution_spectrum,(6.5,7.5))
        Electric_Field_Ey_evolution_1_list.append(get_envelope_width(Electric_Field_1))
        Electric_Field_Ey_evolution_3_list.append(get_envelope_width(Electric_Field_3))
        Electric_Field_Ey_evolution_5_list.append(get_envelope_width(Electric_Field_5))
        Electric_Field_Ey_evolution_7_list.append(get_envelope_width(Electric_Field_7))
        ...
        #write_field_2D(Field_list=[Electric_Field_Ey_evolution],x_axis=window_grid_x_axis,y_axis=window_grid_y_axis,name_list=['Electric_Field_Ey'],nc_name=os.path.join(working_dir,'%s_time=%+05.01fT.nc' %(name,evolution_time/laser_period)))
        #Electric_Field_Ey_evolution_func_list.append(func(Electric_Field_Ey_evolution))
    #return Electric_Field_Ey_evolution_func_list
    pd.DataFrame(Electric_Field_Ey_evolution_1_list).to_hdf(path_or_buf=os.path.join(working_dir,'waist_x.hdf5'),key='1')
    pd.DataFrame(Electric_Field_Ey_evolution_3_list).to_hdf(path_or_buf=os.path.join(working_dir,'waist_x.hdf5'),key='3')
    pd.DataFrame(Electric_Field_Ey_evolution_5_list).to_hdf(path_or_buf=os.path.join(working_dir,'waist_x.hdf5'),key='5')
    pd.DataFrame(Electric_Field_Ey_evolution_7_list).to_hdf(path_or_buf=os.path.join(working_dir,'waist_x.hdf5'),key='7')
for n in [1,3,5,7]:
    print(n)
    a=pd.read_hdf(path_or_buf=os.path.join(working_dir,'waist_x.hdf5'),key='%d'%n)
    #print(a.columns)
    print(a['Electric_Field_envelope_max_id'])

    evolution_time_list=np.linspace(-30*laser_period,0*laser_period,31,endpoint=True)
    xy_list=[]
    for i,evolution_time in enumerate(evolution_time_list):
        window_grid_x_axis=grid_x_axis-C.speed_of_light*evolution_time
        xy_list.append({'x':window_grid_x_axis[a['Electric_Field_envelope_max_id'][i][0]],
                        'y':grid_y_axis[a['Electric_Field_envelope_max_id'][i][1]]})
    pd.DataFrame(xy_list).to_hdf(path_or_buf=os.path.join(working_dir,'waist_x.hdf5'),key='%d_xy'%n)
exit(0) 

#data_dict=read_nc(nc_name=os.path.join(working_dir,'0001_transmission_rotate_500cpl.nc'),key_name_list=['Electric_Field_Ex','Electric_Field_Ey','Magnetic_Field_Bz','x','y'])
data_dict=read_sdf(sdf_name=os.path.join(working_dir,'0025.sdf'),block_name_list=['Electric_Field_Ex','Electric_Field_Ey','Magnetic_Field_Bz'])
...
evolution_time_list=np.linspace(-30*laser_period,0*laser_period,31,endpoint=True)
get_evolution(Electric_Field_Ex=Electric_Field_Ex,Electric_Field_Ey=Electric_Field_Ey,Magnetic_Field_Bz=Magnetic_Field_Bz,evolution_time_list=evolution_time_list,window_shift_velocity=(-C.speed_of_light,0,0),name='reflection')
exit(0)

Electric_Field_Ex_spectrum=get_spectrum(Electric_Field_Ex,'Electric_Field_Ex')
Electric_Field_Ey_spectrum=get_spectrum(Electric_Field_Ey,'Electric_Field_Ey')
Magnetic_Field_Bz_spectrum=get_spectrum(Magnetic_Field_Bz,'Magnetic_Field_Bz')
evolution_time_list=np.linspace(-30*laser_period,10*laser_period,41,endpoint=True)
for o in [5]:
    Electric_Field_Ex_3,_=filter_spectrum(Electric_Field_Ex_spectrum,(o-0.5,o+0.5))
    Electric_Field_Ey_3,_=filter_spectrum(Electric_Field_Ey_spectrum,(o-0.5,o+0.5))
    Magnetic_Field_Bz_3,_=filter_spectrum(Magnetic_Field_Bz_spectrum,(o-0.5,o+0.5))

    Electric_Field_Ey_evolution_func_list=get_evolution(Electric_Field_Ex=Electric_Field_Ex_3,Electric_Field_Ey=Electric_Field_Ey_3,Magnetic_Field_Bz=Magnetic_Field_Bz_3,evolution_time_list=evolution_time_list,window_shift_velocity=(C.speed_of_light,0,0),func=get_envelope_width,name='%d'%o)
    Electric_Field_Ey_evolution_envelope_max_list=[]
    Electric_Field_Ey_evolution_envelope_centercross_peak_left_list=[]
    Electric_Field_Ey_evolution_envelope_centercross_peak_right_list=[]
    Electric_Field_Ey_3_max_x_list=[]
    for Electric_Field_Ey_evolution_envelope_dict, evolution_time in zip(Electric_Field_Ey_evolution_func_list,evolution_time_list):
        window_grid_x_axis=grid_x_axis+C.speed_of_light*evolution_time
        Electric_Field_envelope_max_id=Electric_Field_Ey_evolution_envelope_dict['Electric_Field_envelope_max_id']
        Electric_Field_Ey_3_max_x_list.append(window_grid_x_axis[Electric_Field_envelope_max_id[0]])
        Electric_Field_Ey_evolution_envelope_max_list.append(Electric_Field_Ey_evolution_envelope_dict['Electric_Field_envelope_max'])
        Electric_Field_Ey_evolution_envelope_centercross_peak_left_list.append(Electric_Field_Ey_evolution_envelope_dict['Electric_Field_envelope_centercross_peak_left'])
        Electric_Field_Ey_evolution_envelope_centercross_peak_right_list.append(Electric_Field_Ey_evolution_envelope_dict['Electric_Field_envelope_centercross_peak_right'])
    pd.DataFrame(data=Electric_Field_Ey_evolution_envelope_max_list,index=Electric_Field_Ey_3_max_x_list).to_hdf(path_or_buf=os.path.join(working_dir,'waist_x.hdf5'),key='transmission_envelope_max_%d' %o)
    pd.DataFrame(data=Electric_Field_Ey_evolution_envelope_centercross_peak_left_list,index=Electric_Field_Ey_3_max_x_list).to_hdf(path_or_buf=os.path.join(working_dir,'waist_x.hdf5'),key='transmission_envelope_centercross_peak_left_%d' %o)
    pd.DataFrame(data=Electric_Field_Ey_evolution_envelope_centercross_peak_right_list,index=Electric_Field_Ey_3_max_x_list).to_hdf(path_or_buf=os.path.join(working_dir,'waist_x.hdf5'),key='transmission_envelope_centercross_peak_right_%d' %o)
exit(0)
